...
<%
/*
모든영역(페이지, 리퀘스트, 세션, 어플리케이션)에는 모든 타입의 객체를 저장할 수 있다.
Object타입으로 자동 형변환되어 저장되고, 사용시에는 각 객체로 형변환후 사용하면 된다.
차후 EL(표현언어)을 사용하면 형변환없이 사용할 수 있다.

1. page영역
	- 페이지영역에 저장된 속성은 해당 페이지에서만 공유되고, 페이지를 벗어나는 순간 소멸된다.
...
pageContext.setAttribute("pageMember2", 
		new MemberDTO("Nakfa", "1234", "성강사", null));
%>
<!DOCTYPE html>
<html>
<head>
...
<body>
	...
	<%
	/*
	page영역에 저장될 때 Object타입으로 자동형변환되어 저장되므로 읽어올떄도
	...
	*/
	...
	<%--
		페이지영역에 저장된 속성값은 새로운페이지에 대한 요청이 발생하면 속성값이 모두
		파괴되어 소멸된다.
	...
	<br />
	<%--
		외부파일을 인클루드 하는경우 외부문서를 원본그대로 현재문서에 포함된 후
