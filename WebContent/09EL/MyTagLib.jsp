...
<%-- 
  	...
  	2. TLD파일을 생성한다. Tag Library Descriptor의 약자로 web-inf폴더 하위 아무곳이나 생성하면 컨테이너가 찾아서 사용하게 된다.
  	우리는 tlds 폴더를 생성한후 해당 파일을 작성한다.
  	파일 작성시 fn.tld를 참조한다.
  	
  	3. web.xml에 내가 정의한 태그라이브러리 파일을 설정한다.
  		taglib-uri : JSP의 taglib지시어의 uri속성에 삽입할 값
  		taglib-lication : 실제 tld 파일의 위치(경로를 삽입함)
  	
  	4. jsp파일에서 EL식으로 메소드를 호출한다.
  		페이지 상단에 taglib 지시어를 통한 페이지 지시어를 통한 디렉티브를 추가한 후 ${접두어:메소드명(인자)} 형태로 호출한다.
 --%>
<!DOCTYPE html>
<html>
<head>
