...
<body>
	...
	<fieldset>
		...
		<!-- 
			EL은 null에 대해 어느정도까지는 처리를 해주므로 아래는 에러가 발생하지는 않는다.
		 -->
		<h2>특수문자가 포함된 속성에 .으로 접근시</h2>
		<ul>
			<li>\${header["user-agent"] }
					 : ${header["user-agent"] } [정상]</li>
			<li>\${header.user-agent } : ${header.user-agent } [에러NO, 0출력]</li>
		</ul>
		...
		<!-- JSTL의 forEach 태그를 통해 반복출력. 해당 문장은 배열이나 컬렉션의 크기만큼
			반복하는 확장for문의 형태로 기술됨 -->
		...
		<c:forEach items="${elCollection}" var="mountain"> ${mountain}&nbsp;
		</c:forEach>
		...
		<ul>
			...
			<!-- 
				EL을 통해 4가지 영역에 접근할때는 XXXScope 패턴으로 사용한다.
		...
		<h2>EL변수에 값 할당시 [톰캣 8.0부터는 정상. 이전버전은 에러]</h2>
