...
<%-- 
	JSP
	- 스크립틀릿 (<% 자바 소스코드 %>) : 자바 코드 쓴다..
	- 표현식 (<%= 값 또는 변수 %>) : = out.println()
	- 선언문 (<%! 맴버변수, 메서드  %>) : 맴버변수와 메스드를 선언
	- 주석문 (<%-- 주석 내용 --%>)
 --%>
...
<body>
	...
	*/
	...
	<!-- 
		최초페이지를 실행했을때에는 파라미터가 없으므로 0으로 간주되어 계산됨
		만약 
			해당페이지?myNumber=20인 경우 결과 30출력됨
			해당페이지?myNumber= 인 경우 0으로 간주되어 10출력됨
	...
	/*
		...
		JSP의 변수를 EL에서 사용할 수 없는 이유는 EL은 4가지 영역에 저장된 속성들만 사용하기 떄문이다
	...
	*/
	...
	<!-- 
		...
		set태그를 이용해서 변수를 선언한다. JSP에서 즉시 선언하려면 영역에 저장한다.
	 -->
	...
	<!-- 
		...
		하지만 개발시에는 실제 서비스 할 웹서버의 버전을 확인 후 사용여부를 결정한다.
		EL은 전통적으로 값을 표현(출력)하는 용도로 사용되어 졌으므로 표현용으로만 사용하는것이 좋다.
	 -->
	<h3>EL변수에 값 할당</h3>
