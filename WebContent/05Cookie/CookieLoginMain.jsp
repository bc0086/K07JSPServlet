...
<body>
	...
	<span style="color:red; font-size:1.5em;">
		로그인처리메세지 출력부분:
		<%=request.getAttribute("ERROE_MSG")==null?
...
%>
	...
	<!-- 
		input태그의 tabindex속성
		: 포커스 이동을 위해 탭(Tab)키를 눌렀을때 포커스가 이동하는 순서를 지정한다.
	...
	<table border="1">
		<tr>
			<td>아이디</td>
			<td>
				<!-- 변수user에 저장된 값이 있다면 value속성에 삽입 -->
				<input type="text" name="user_id" tabindex="1"
					value="<%= user==null ? "" : user %>" />
					
				<!-- 변수user에 저장된 값이 있을때 checkbox의 checked속성을 
					활성화한다. -->
...
<%
	...
	// 세션영역에 저장된 속성값이 있다면 로그인성공 화면을 보여준다.
%>
	<table border=1 width=400>
		<tr>
			<td align="center">
			<!-- 세션영역에 저장된 속성이 있으므로 getAttribute로 읽어올 수 있다. -->
				<%=session.getAttribute("USER_ID") %>님, 로그인하셨습니다.<br />
				즐거운 시간 보내세요
				<br />
