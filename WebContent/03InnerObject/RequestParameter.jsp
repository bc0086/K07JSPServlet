...
<body>
	...
	/*
	전송방식 get/post에 상관없이 getParameter('파라미터명")메소드로 폼값을 받을 수 있다 
	만약 값이 입력되지 않았다면 길이가 0인 String객체를 반환하고,
	파라미터명이 틀린경우에는 null값이 반환된다.
	...
	*/
	...
	if( !(name==null || name.equals("")) ){
		...
		아래와 같이 사용할 수 있다. 또는 문자열을 연결하여 사용할수도 있다.
		*/
		out.println("<script>");
	...
	/*
	radio는 폼값을 아래 2가지 방법을 모두 사용할 수 있다.
	항목은 여러개이나, 실제 선택가능한 항목은 하나이기 때문이다.
	*/
	
	//방법1 : text타입과 동일한 형태로 폼값을 받을 때
	...
	여러개의 항목중 체크가 된 항목만 전송된다. 파라미터명이 클리거나
	...
	*/
	...
	if(favo != null){
		for(int i=0; i<favo.length; i++){
			...
			문자열의 마지막 부분에 ,를 제거하기 위해 if문을 통해 체크하고 있다.
			*/
			if(i==favo.length-1){
				favStr += favo[i];
			}
	...
	웹브라우저에 출력할땐는 특수기호를 <br>태그로 변환 후 출력해야 한다.
	*/
	String self_intro =
