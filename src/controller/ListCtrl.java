...
public class ListCtrl extends HttpServlet {
		...
		protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
			...
			// 전체 카운트 수를 Map에 저장함. 차후 View로 전달할 예정임
			param.put("totalCount", totalRecordCount);
			
			/////////////////////////////////////////////////
			// 페이지 수 계산을 위한 페이지설정값 가져오기
			...
			// 테이블의 해당페이지 구간의ResultSet을 가져옴.(페이지 처리 o)
		...
		protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
			...
			/*
			 	만약 게시판 리스트쪽에서 POST방식으로 요청이 들어오더라도 처리는
			 	doGet() 메소드에서 처리할 수 있도록 모든 요청을 토스한다.
			 */
			doGet(req, resp);
		}
}
