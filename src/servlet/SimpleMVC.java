...
/*
	서블릿 호출을 위한 순서
	1. 요청명을 결정한다.
	2. 요청명에 대한 매핑처리를 한다. 이떄 요청명과 해당 요청을
		처리할 서블릿 클래스가 매핑된다.
	3. 사용자의 요청이 서블릿으로 전달되고 서블릿은 요청을 분석한다.
	4. 분석한 요청에 따라 Model(모델)을 사용하여 기능을 수행한다.
	5. 기능을 수행한 후 필요한 데이터를 request 혹은 session 영역에 저장한다.
...
public class SimpleMVC extends HttpServlet {
	...
	/*
	  	doGet() 혹은 doPost()의 요청을 그대로 전달받기 위해서는 아래와같이 메소드를 기술해야 한다.
	  	request, response객체를 매개변수로 사용하고 두가지에 대한 예외를 반드시 처리해야한다.
	 */
	...
	throws ServletException, IOException {
		...
		if(type == null) {
			...
			/*
			 	서블릿 클래스에서 application 내장객체를 사용하기 위해 getServletContext()
			 	메소드를 이용해서 가져온다.
			 */
